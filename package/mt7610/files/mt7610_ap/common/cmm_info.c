...
/*	RT_802_11_MAC_TABLE MacTab;*/
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
		{
			...
			getRate(pEntry->HTPhyMode, &(rate));						
			pDst->Rate = rate;
			sprintf(tmpstr,"%s",GetPhyMode_dlink(pEntry->HTPhyMode.field.MODE,pAd->CommonCfg.PhyMode));
			
			memcpy(pDst->ConnectionWord,tmpstr,strlen(tmpstr));
			DBGPRINT(RT_DEBUG_ERROR, ("%s:pDst->ConnectionWord=%s ,pEntry->RssiSample.AvgRssi0=%d , pEntry->RssiSample.AvgRssi1=%d\n", __FUNCTION__,pDst->ConnectionWord,pEntry->RssiSample.AvgRssi0,pEntry->RssiSample.AvgRssi1));
			pDst->Rssi = GetSignal_dlink(pEntry->RssiSample.AvgRssi0,pEntry->RssiSample.AvgRssi1);
			DBGPRINT(RT_DEBUG_ERROR, ("%s:pMacTab->Num =%d\n", __FUNCTION__,pMacTab->Num));
	...
	IN	PSTRING			*arg)
{
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
			...
			&& (pEntry->Sst == SST_ASSOC))
		{
			...
			//printk("%-7d", pEntry->RssiSample.AvgRssi1);
			//printk("%-7d", pEntry->RssiSample.AvgRssi2);
			printk(" %s", GetPhyMode_dlink(pEntry->HTPhyMode.field.MODE,pAd->CommonCfg.PhyMode));
			//printk("%-6s", GetBW(pEntry->HTPhyMode.field.BW));
			//printk("%-6d", pEntry->HTPhyMode.field.MCS);
			//printk("%-6d", pEntry->HTPhyMode.field.ShortGI);
			//printk("%-6d", pEntry->HTPhyMode.field.STBC);
			//printk("%-7d", (int)(pEntry->StaIdleTimeout - pEntry->NoDataIdleCount));
			printk(" %dM", (int)DataRate);
			printk(" %d%", GetSignal_dlink(pEntry->RssiSample.AvgRssi0,pEntry->RssiSample.AvgRssi1));
			printk("\n");
		}
	} 
