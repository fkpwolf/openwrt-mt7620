...
static struct {
	...
	{"stainfo_dlink",			Show_MacTable_dlink_Proc},	/* cameo brian add */
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		case RT_OID_802_11_WSC_QUERY_PROFILE:
			...
			if (pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeWPA1PSKWPA2PSK){
				pProfile->Profile[0].AuthType = WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK;
				DBGPRINT(RT_DEBUG_TRACE, ("=>Bug On patch Brian!\n"));
			}
			if (pAd->ApCfg.MBSSID[apidx].WepStatus == Ndis802_11Encryption4Enabled){
				pProfile->Profile[0].EncrType = WSC_ENCRTYPE_TKIP |WSC_ENCRTYPE_AES;
				DBGPRINT(RT_DEBUG_TRACE, ("=>Bug On patch Brian!\n"));
			}
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Query::RT_OID_802_11_WSC_QUERY_PROFILE apidx=%d pProfile->Profile[0].AuthType=%02x,pAd->ApCfg.MBSSID[apidx].Authmode=%02x\n",apidx,pProfile->Profile[0].AuthType,pAd->ApCfg.MBSSID[apidx].AuthMode));
